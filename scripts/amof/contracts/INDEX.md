# Contracts Index

Status: canonical

## Canonical

- [README.md](README.md)
- [validation-closure.v1.md](validation-closure.v1.md)
- [director-intake-client-contract.md](director-intake-client-contract.md)
- [director-intake-execution-contract.schema.json](director-intake-execution-contract.schema.json)
- [director-plan-result.schema.json](director-plan-result.schema.json)
- [workspace-receipt.schema.json](workspace-receipt.schema.json)
- [context-assembly-receipt.v1.schema.json](context-assembly-receipt.v1.schema.json)
- [execution-handoff-result.schema.json](execution-handoff-result.schema.json)
- [write-scope-proposal.schema.json](write-scope-proposal.schema.json)
- [write-scope-approval.schema.json](write-scope-approval.schema.json)
- [write-scope-binding.schema.json](write-scope-binding.schema.json)
- [write-scope-revocation.schema.json](write-scope-revocation.schema.json)
- [mutation-receipt.schema.json](mutation-receipt.schema.json)
- [kubernetes-capability-proposal.schema.json](kubernetes-capability-proposal.schema.json)
- [kubernetes-capability-approval.schema.json](kubernetes-capability-approval.schema.json)
- [kubernetes-capability-binding.schema.json](kubernetes-capability-binding.schema.json)
- [kubernetes-capability-revocation.schema.json](kubernetes-capability-revocation.schema.json)
- [kubernetes-capability-receipt.schema.json](kubernetes-capability-receipt.schema.json)
- [reference-capability-receipt.schema.json](reference-capability-receipt.schema.json)
- [public-promotion-proof.schema.json](public-promotion-proof.schema.json)
- [external-agent-plan-execute-request.schema.json](external-agent-plan-execute-request.schema.json)
- [examples/external-agent-plan-execute-request.example.json](examples/external-agent-plan-execute-request.example.json)
- [examples/external-agent-plan-execute-request.minimal.example.json](examples/external-agent-plan-execute-request.minimal.example.json)
- [examples/external-agent-plan-execute-request.invalid-unsafe.example.json](examples/external-agent-plan-execute-request.invalid-unsafe.example.json)
- [governed-workstation-bootstrap-contract.schema.json](governed-workstation-bootstrap-contract.schema.json)
- [bootstrap-source-checkout-receipt.schema.json](bootstrap-source-checkout-receipt.schema.json)
- [bootstrap-toolchain-receipt.schema.json](bootstrap-toolchain-receipt.schema.json)
- [bootstrap-provider-configuration-receipt.schema.json](bootstrap-provider-configuration-receipt.schema.json)
- [bootstrap-failure-receipt.schema.json](bootstrap-failure-receipt.schema.json)
- [up10-bootstrap-summary.schema.json](up10-bootstrap-summary.schema.json)
- [bootstrap-sha256-manifest.schema.json](bootstrap-sha256-manifest.schema.json)
- [examples/director-intake-dirty-workspace.example.json](examples/director-intake-dirty-workspace.example.json)
- [examples/director-intake-source-fix-ticket.example.json](examples/director-intake-source-fix-ticket.example.json)
- [examples/authority-ledger-bounded-action.example.json](examples/authority-ledger-bounded-action.example.json)
- [examples/authority-ledger-refuse.example.json](examples/authority-ledger-refuse.example.json)
- [examples/intake-authority-evaluation-bounded-action.example.json](examples/intake-authority-evaluation-bounded-action.example.json)
- [examples/runner-authority-gating-bounded-match.example.json](examples/runner-authority-gating-bounded-match.example.json)
- [examples/governed-workstation-bootstrap-pass.example.json](examples/governed-workstation-bootstrap-pass.example.json)
- [examples/governed-workstation-bootstrap-warn.example.json](examples/governed-workstation-bootstrap-warn.example.json)
- [examples/governed-workstation-bootstrap-blocked.example.json](examples/governed-workstation-bootstrap-blocked.example.json)
