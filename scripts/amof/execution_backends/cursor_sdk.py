"""Cursor SDK backend contract for governed AMOF handoffs.

Worker / delegated implementation backend only. Runtime Authority, IAL
gateway, and operator-console core must never import ``cursor_sdk`` /
...
BACKEND_CONTRACT_VERSION = "cursor-sdk-v1"
RUNTIME_CONTRACT = "Cursor SDK (local Agent.create/send) + CURSOR_API_KEY"
...
class CursorSdkBackendError(RuntimeError):
    """Raised when the Cursor SDK backend cannot be dispatched truthfully."""


@dataclass(frozen=True)
class CursorSdkInvokeResult:
...
def _requested_model(model_override: str | None = None) -> str:
    return (
        str(model_override or os.environ.get("AMOF_CURSOR_SDK_MODEL") or "").strip()
        or DEFAULT_MODEL
    )


def _setting_sources() -> tuple[str, ...]:
    raw = str(os.environ.get("AMOF_CURSOR_SDK_SETTING_SOURCES") or "").strip()
    if not raw:
...
def is_cursor_sdk_runner(record: dict[str, Any]) -> bool:
...
def runtime_health() -> dict[str, Any]:
    ...
    flag = str(os.environ.get("AMOF_CURSOR_SDK_ENABLED") or "1").strip().lower()
    ...
    return {
        ...
        "cursor_sdk_runtime": "ready" if sdk_ready else "unavailable",
        ...
        "execution_endpoint": "cursor_sdk.Agent",
...
def doctor_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cursor_sdk_runtime": health.get(
            "cursor_sdk_runtime", health["runtime_health"]
        ),
...
def _run_dir(run_id: str) -> Path:
    path = runs_dir() / "cursor-sdk" / run_id
...
) -> CursorSdkInvokeResult:
    """Invoke Cursor SDK local runtime. Patchable for unit tests (no live key)."""
    try:
...
def run(
    *,
    ...
    if not dispatch_probe:
        dispatch_probe = _probe_sdk_import()
    _shared._append_event(event_log_path, "cursor_sdk_dispatch_probe", **dispatch_probe)

    if provider and provider not in {PROVIDER, "cursor-sdk", "cursor"}:
        return _blocked_result(
            run_id=run_id,
            stop_reason="inference_transport_unavailable",
            final_text="Provider override is not allowed for the AMOF-managed Cursor SDK runner.",
            studio_session_id=studio_session_id,
            event_log_path=event_log_path,
            runtime_log_path=runtime_log_path,
            result_path=result_path,
    ...
    if not _api_key():
        return _blocked_result(
            run_id=run_id,
            stop_reason="inference_transport_unavailable",
            final_text="Cursor API credential (CURSOR_API_KEY) is not configured for the Cursor SDK runner.",
            studio_session_id=studio_session_id,
            event_log_path=event_log_path,
            runtime_log_path=runtime_log_path,
            result_path=result_path,
    ...
    if not health.get("feature_flag_enabled", True):
        return _blocked_result(
            run_id=run_id,
            stop_reason="cursor_sdk_dispatch_unavailable",
            final_text="Cursor SDK backend is disabled by AMOF_CURSOR_SDK_ENABLED.",
            studio_session_id=studio_session_id,
            event_log_path=event_log_path,
            runtime_log_path=runtime_log_path,
            result_path=result_path,
    ...
    if dispatch_probe["status"] != "ready":
        return _blocked_result(
            run_id=run_id,
            stop_reason="cursor_sdk_dispatch_unavailable",
            final_text="Cursor SDK package is unavailable; selected runner failed closed.",
            studio_session_id=studio_session_id,
            event_log_path=event_log_path,
            runtime_log_path=runtime_log_path,
            result_path=result_path,
    ...
    while True:
        ...
        try:
            ...
            if invoke.status == "startup_failed":
                ...
                status = "failed"
                stop_reason = "cursor_sdk_run_failed"
                exit_code = 2
        except Exception as exc:
            status = "failed"
            stop_reason = "cursor_sdk_runtime_exception"
            exit_code = 1
...
def _result_payload(
    *,
    ...
    if isinstance(sdk_envelope, dict) and isinstance(sdk_envelope.get("usage"), dict):
        # Cursor SDK usage is token-shaped; dollar spend is not always present.
    ...
    return {
        ...
        **(
        ...
        "validation_summary": {
            ...
            "reason": (
                "Cursor SDK backend returns process status; focused validation "
...
def _runtime_summary_text(
    *,
    ...
    return (
        f"AMOF Cursor SDK run {run_id} finished with status={status}, "
